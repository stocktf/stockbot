    ...
    if (isReceiveJob) {
      ...
      getItems(function(items) {
        ...
        if (keysInStock.length > 0) {
          ...
          // Add key to the bots inventory
          redis.sadd(games[game] + ':keys:inventory', keysInStock, function (err, updated) {
            ...
            log.warn('Added %s key(s) to inventory', updated);

          });

          // Add key to main pool
          redis.sadd(games[game] + ':keys', keysInStock, function (err, updated) {
            ...
            log.warn('Added %s key(s) to the main pool', updated);
