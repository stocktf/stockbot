...
var gcDownHandler = require('./gcDown');
var quHandler = require('./quHandler');
...
module.exports = function(bot, botTrade, botOffers) {
  ...
  bot.on('tradeResult', tradeResultHandler);
  ...
  bot.on('quReady', quHandler);

  bot.on('quJob', quJobHandler);

  /*
   *  Handle gc errors
   */

  bot.on('gcDown', function (reloadInv) {
  
    gcDownHandler(bot, botTrade, reloadInv);  
